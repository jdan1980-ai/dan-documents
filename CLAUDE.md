...
- **Every Shorts script MUST include a thumbnail prompt (Nano Banana 2) + thumbnail negatives.** Locked 26 мая 2026 — user directive ("Добавляй промт для тумбнейл в скрипты для каждого шортса"). No script ships without it. Lives in the SEO Pack section right after the description/tags, before the Pre-flight block. The template's thumbnail spec already has the channel-locked typography (Fredoka One / Nunito Bold, Electric Yellow #FFD23F, charcoal outline) + anatomy-eye rule + anti-realism wording — reuse it from `script-template.md`.
