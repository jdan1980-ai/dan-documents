...
> 🕒 **Publish-slot rule (1 июн 2026, data-validated по API):** **Идеальный слот = 13:00-14:00 локального TZ (Jerusalem UTC+3)** — это тот таймслот в котором публиковались апрельские шортсы 24 апр – 5 мая ДО падения трафика (когда канал стабильно делал 700-1500 v/day). Алгоритм помнит "аудитория активна в эти часы" по тем видео которые тогда работали. **Допустимое окно: 13:00-15:00 локального.** 15:00 = late fallback когда user не успевает к 14:00; не предлагать слоты позже 15:00 пока recovery не подтверждён (subs растут + Shorts-feed retention > 30%). 18:00-21:00 = период падения трафика (10 мая – 27 мая), НЕ возвращаемся туда. **Default рекомендация для нового шортса = 14:00 локального.**
