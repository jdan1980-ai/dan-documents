# Script — A Cat Went to SPACE?! 5 Famous Cats Who Made History 🐱 | Cat Facts

**Формула:** killer-листикл (mind-blow records) + warm close. Brain = ведущий и «играет» каждого знаменитого кота (костюм + подпись) — реальных котов НЕ показываем (канал = один анимированный Brain).
**Пайплайн (LOCKED):** image в **Nano Banana 2** → анимация в **Kling 3.0 (image-to-video)** → оверлеи/цифры/текст + сборка в **CapCut** → VO **Google Vids TTS**. Статичные сцены — Ken Burns.

---

## Meta
- **Title (working):** A Cat Went to SPACE?! 5 Famous Cats Who Made History 🐱 | Cat Facts
- **Slug:** `famous-cats-history`
- **Category:** Cat Facts (mind-blow listicle / history)
- **Series:** killer-формула (листикл-рекорды)
- **Status:** script
- **Publish target:** 29 мая 2026
- **vidIQ:** main keyword **`top 10 cats` 71.41 / 11,518 / comp 12.4 (очень низкая) = GREEN-jackpot**. Seed `famous cats` 57.85/5,099. Cached 2026-05-28. Channel-wide tag `cat facts mind blowing` 73.8 ⭐.

## Audience Promise
Зритель узнаёт 5 реальных удивительных историй знаменитых котов (первый кот в космосе, переживший 3 кораблекрушения, кошка-начальник станции, мышелов резиденции премьера Британии, кот-мэр). Эмоция: «не знал, что коты ТАКОЕ делали» + улыбка + шер.

## ✍️ Killer-формула в этом скрипте
- **Mind-blow hook Sc 1:** «один кот летал в космос, другой пережил 3 кораблекрушения» — open-loop
- **Нумерованный листикл** (5 котов) — retention
- **Specific spikes:** имена/годы/места (Фелисетт 1963, 3 корабля, Япония, 10 Downing St, 20 лет)
- **🔧 Warm twist Sc 7:** «кот может изменить мир, просто оставаясь собой»
- **🔧 Open-loop CTA Sc 8:** «может, следующий знаменитый кот спит на твоём диване»

---

## 🔒 Locked Brain Prompt (вшит в каждый image-промт — Nano Banana 2)
```
Cute Pixar/Disney 3D ANIMATED CARTOON orange tabby kitten named Brain (8-10 week old kitten, slender, NOT adult, NOT chubby, NOT photorealistic, NOT a real cat), big round Pixar-style eyes — each eye has a LARGE PURE WHITE sclera (white of the eye clearly visible all around) surrounding a medium round EMERALD-GREEN iris (#3DDC84) with a black pupil in the center; ONLY the small iris is colored green, the white of the eye stays pure white, NEVER a fully-green eyeball, NEVER green sclera. Small thin round gold-framed glasses ALWAYS on. A plain smooth brown leather collar (smooth band, NO studs, NO buckle) with EXACTLY ONE clearly HEART-SHAPED gold tag (unmistakable heart silhouette) engraved "Brain" (NOT round, NOT a disc, NOT a bell). Soft fluffy orange tabby fur with darker stripes, ALL 4 paws — both FRONT and both BACK paws — fully ginger tabby (NO white socks, NO white back paws, NO white toes), tail ginger to the very tip (NO white tip), pink nose, white whiskers, EXACTLY 2 ears, EXACTLY 4 paws, mouth closed. Vertical 9:16.
```
**Negatives (универсальные):**
```
photorealistic, real cat, blue eyes, amber eyes, brown eyes, green sclera, green eye-whites, fully green eyes, green-tinted eyeballs, white paws, white socks, white back paws, white hind paws, white back feet, white tail tip, round tag, oval tag, bell tag, coin tag, blank tag, studded collar, missing glasses, three ears, extra paw, mouth open, second cat, second kitten, two cats, on-screen text, watermark
```

## 🎬 Kling animation (image-to-video) — суффикс в каждый motion-промт
```
Preserve the input image exactly — same kitten, same anatomy, glasses, eyes (large white sclera with small emerald iris), collar + heart tag, ginger paws and tail, costume, Pixar cartoon style. Only animate the motion described — do NOT redraw, restyle, recolor or morph. Mouth closed. Vertical 9:16.
```

## 🔢 LOCKED numeral/text style (CapCut)
Цифры «1–5» и подписи имён — в CapCut одним залоченным стилем: Fredoka One / Nunito Bold, Electric Yellow #FFD23F, charcoal outline, drop-shadow.

## 👕 Костюм-правило
Каждый костюм НЕ закрывает ошейник — оставляем коричневый ошейник с золотым сердечком «Brain» видимым у выреза. Костюм = один яркий узнаваемый элемент (шлем / бескозырка / фуражка / и т.д.).

---

## 🏠 Локации
**Host (Sc 1, 7) — INT. COZY LIVING ROOM — DAY:** наша залоченная гостиная — cream walls, honey-amber wood floor, ONE plain SOLID sage-green woven rug (no pattern, no border, no fringe), cream armchair RIGHT, side-table LEFT, tall window BACKGROUND with sheer curtains, snake plant. Warm daylight.
**Vignettes (Sc 2-6):** отдельные стилизованные мини-сцены (космос / корабль / ж/д станция / дверь резиденции / мэрия) — простые, чистые, мультяшные, фон не перегружать.

---

## Полный VO

**🇬🇧 English (copy this block alone):**
```
Some cats are more famous than movie stars. One flew to space, another survived three shipwrecks. Here are five legends.

Number one — Félicette. In 1963, France launched her into space. She's the only cat who has ever reached the stars.

Number two — Unsinkable Sam. He survived not one, not two, but three ships sinking under him in World War Two.

Number three — Tama. This little cat became a train station master in Japan, and saved the whole railway from closing.

Number four — Larry. He lives at the British Prime Minister's house with an official job title: Chief Mouser.

Number five — Stubbs. He was the honorary mayor of a town in Alaska for twenty years.

These cats prove one thing — a cat can change the world just by being itself.

Subscribe — maybe the next famous cat is sleeping on your couch right now.
```

**🇷🇺 Русская версия (copy this block alone):**
```
Некоторые коты знаменитее кинозвёзд. Один летал в космос, другой пережил три кораблекрушения. Вот пять легенд.

Номер один — Фелисетт. В 1963-м Франция отправила её в космос. Она единственная кошка, побывавшая среди звёзд.

Номер два — Непотопляемый Сэм. Он пережил не одно, не два, а целых три кораблекрушения во Вторую мировую.

Номер три — Тама. Эта маленькая кошка стала начальником ж/д станции в Японии и спасла дорогу от закрытия.

Номер четыре — Ларри. Он живёт в доме премьер-министра Британии с официальной должностью: главный мышелов.

Номер пять — Стаббс. Он двадцать лет был почётным мэром городка на Аляске.

Эти коты доказали одно — кот может изменить мир, просто оставаясь собой.

Подпишись — может, следующий знаменитый кот спит на твоём диване прямо сейчас.
```
_~115 слов EN, ~7с/сцена._

---

# Scenes

## Сцена 1 — ХУК (host) — 0–7с
**Что зритель видит:** Brain-ведущий в гостиной, восторженно представляет тему; рядом парят маленькие иконки (звезда/ракета/медаль). Text-плита «5 FAMOUS CATS». VO: один летал в космос, другой пережил 3 кораблекрушения.

**🎨 Image (Nano Banana 2):**
```
Cute Pixar/Disney 3D ANIMATED CARTOON orange tabby kitten named Brain (8-10 week old kitten, slender, NOT adult, NOT chubby, NOT photorealistic, NOT a real cat), big round Pixar-style eyes — each eye has a LARGE PURE WHITE sclera (white of the eye clearly visible all around) surrounding a medium round EMERALD-GREEN iris (#3DDC84) with a black pupil; ONLY the small iris is green, the white of the eye stays pure white, NEVER a fully-green eyeball. Small thin round gold-framed glasses ALWAYS on. A plain smooth brown leather collar (smooth band, NO studs, NO buckle) with EXACTLY ONE clearly HEART-SHAPED gold tag engraved "Brain" (NOT round, NOT a disc). Soft fluffy orange tabby fur, ALL 4 paws — both FRONT and both BACK paws — fully ginger tabby (NO white socks, NO white back paws), tail ginger to the tip, EXACTLY 2 ears, EXACTLY 4 paws, mouth closed in a bright excited host expression. INT. COZY LIVING ROOM — DAY (same locked room: cream walls, honey-amber wood floor, ONE plain SOLID sage-green woven rug — no pattern, no border, no fringe — cream armchair, tall window with sheer curtains, snake plant). Medium shot. Brain sits upright on the sage-green rug presenting to camera with an eager "you won't believe this" look, one paw raised in a presenting gesture. Floating around him: small cute cartoon icons — a tiny rocket, a gold star, a little medal — and soft sparkles. Negative space upper-third for "5 FAMOUS CATS" text plate. Vertical 9:16.
```
**🎬 Kling (image-to-video):**
```
Brain gives an eager presenting gesture with one paw, eyes bright, the little rocket / star / medal icons gently bob and twinkle around him. Slight push-in. Preserve the input image exactly — same kitten, same anatomy, glasses, eyes (large white sclera with small emerald iris), collar + heart tag, ginger paws and tail, Pixar cartoon style. Only animate the motion described — do NOT redraw, restyle, recolor or morph. Mouth closed. Vertical 9:16.
```
**🎙️ VO (EN):**
```
Some cats are more famous than movie stars. One flew to space, another survived three shipwrecks. Here are five legends.
```
**🎙️ VO (RU):**
```
Некоторые коты знаменитее кинозвёзд. Один летал в космос, другой пережил три кораблекрушения. Вот пять легенд.
```
**Оверлей (CapCut):** «5 FAMOUS CATS» на 2с + тёплый «sparkle» SFX.

---

## Сцена 2 — №1 ФЕЛИСЕТТ (космос) — 7–14с
**Что зритель видит:** Brain в маленьком космическом шлеме (ошейник виден), на фоне звёзды/планета. Подпись «FÉLICETTE · 1963». Цифра «1». VO: первый и единственный кот в космосе.

**🎨 Image (Nano Banana 2):**
```
Cute Pixar/Disney 3D ANIMATED CARTOON orange tabby kitten named Brain (8-10 week old kitten, slender, NOT adult, NOT chubby, NOT photorealistic, NOT a real cat), big round Pixar-style eyes — each eye has a LARGE PURE WHITE sclera (white of the eye clearly visible all around) surrounding a medium round EMERALD-GREEN iris (#3DDC84) with a black pupil; ONLY the small iris is green, the white of the eye stays pure white, NEVER a fully-green eyeball. Small thin round gold-framed glasses ALWAYS on. A plain smooth brown leather collar (smooth band, NO studs, NO buckle) with EXACTLY ONE clearly HEART-SHAPED gold tag engraved "Brain" (NOT round, NOT a disc) — collar clearly visible at the neckline. Soft fluffy orange tabby fur, ALL 4 paws — both FRONT and both BACK paws — fully ginger tabby (NO white socks, NO white back paws), tail ginger to the tip, EXACTLY 2 ears, EXACTLY 4 paws, mouth closed, awe expression. COSTUME: a cute cartoon white-and-cream astronaut helmet with a clear round visor (pushed up so his face + glasses are fully visible), and a tiny cartoon space-suit collar — the brown collar with heart tag still visible beneath. SETTING: a charming stylized outer-space backdrop — deep blue starry space, a few soft cartoon stars and a small planet in the distance, gentle nebula glow. Brain floats gently weightless, looking around in wonder. Clean simple composition. Vertical 9:16.
```
**🎬 Kling (image-to-video):**
```
Brain floats gently weightless in space, slowly drifting, looking around in wide-eyed wonder, the stars twinkle and the small planet drifts behind him. Slow push-in. Preserve the input image exactly — same kitten, same anatomy, glasses, eyes (large white sclera with small emerald iris), collar + heart tag, ginger paws and tail, astronaut helmet costume, Pixar cartoon style. Only animate the motion described — do NOT redraw, restyle, recolor or morph. Mouth closed. Vertical 9:16.
```
**🎙️ VO (EN):**
```
Number one — Félicette. In 1963, France launched her into space. She's the only cat who has ever reached the stars.
```
**🎙️ VO (RU):**
```
Номер один — Фелисетт. В 1963-м Франция отправила её в космос. Она единственная кошка, побывавшая среди звёзд.
```
**Оверлей (CapCut):** цифра «1» + подпись «FÉLICETTE · 1963» + лёгкий «space whoosh» SFX.

---

## Сцена 3 — №2 НЕПОТОПЛЯЕМЫЙ СЭМ (корабль) — 14–21с
**Что зритель видит:** Brain в матросской бескозырке на палубе корабля, спокойный «видавший виды» вид. Подпись «UNSINKABLE SAM». Цифра «2». VO: пережил 3 кораблекрушения.

**🎨 Image (Nano Banana 2):**
```
Cute Pixar/Disney 3D ANIMATED CARTOON orange tabby kitten named Brain (8-10 week old kitten, slender, NOT adult, NOT chubby, NOT photorealistic, NOT a real cat), big round Pixar-style eyes — each eye has a LARGE PURE WHITE sclera (white of the eye clearly visible all around) surrounding a medium round EMERALD-GREEN iris (#3DDC84) with a black pupil; ONLY the small iris is green, the white of the eye stays pure white, NEVER a fully-green eyeball. Small thin round gold-framed glasses ALWAYS on. A plain smooth brown leather collar (smooth band, NO studs, NO buckle) with EXACTLY ONE clearly HEART-SHAPED gold tag engraved "Brain" (NOT round, NOT a disc) — collar clearly visible. Soft fluffy orange tabby fur, ALL 4 paws — both FRONT and both BACK paws — fully ginger tabby (NO white socks, NO white back paws), tail ginger to the tip, EXACTLY 2 ears, EXACTLY 4 paws, mouth closed, calm brave expression. COSTUME: a cute little white sailor's cap (round naval cap) on his head — the brown collar with heart tag still visible at the neck. SETTING: standing on the wooden deck of a charming cartoon ship, calm blue sea and soft sky behind, a ship's railing and a small lifebuoy ring nearby, a couple of gentle waves. Wholesome, brave, NOT scary, NO sinking, NO danger shown. Vertical 9:16.
```
**🎬 Kling (image-to-video):**
```
Brain stands steady on the ship deck, the sea gently sways behind him, his sailor cap and whiskers move slightly in the breeze, a calm confident look. Gentle push-in. Preserve the input image exactly — same kitten, same anatomy, glasses, eyes (large white sclera with small emerald iris), collar + heart tag, ginger paws and tail, sailor cap costume, Pixar cartoon style. Only animate the motion described — do NOT redraw, restyle, recolor or morph. Mouth closed. Vertical 9:16.
```
**🎙️ VO (EN):**
```
Number two — Unsinkable Sam. He survived not one, not two, but three ships sinking under him in World War Two.
```
**🎙️ VO (RU):**
```
Номер два — Непотопляемый Сэм. Он пережил не одно, не два, а целых три кораблекрушения во Вторую мировую.
```
**Оверлей (CapCut):** цифра «2» + подпись «UNSINKABLE SAM» + мягкий «sea/wave» SFX.

---

## Сцена 4 — №3 ТАМА (ж/д станция) — 21–28с
**Что зритель видит:** Brain в фуражке начальника станции у маленькой японской ж/д станции. Подпись «TAMA · STATION MASTER». Цифра «3». VO: стала начальником станции, спасла дорогу.

**🎨 Image (Nano Banana 2):**
```
Cute Pixar/Disney 3D ANIMATED CARTOON orange tabby kitten named Brain (8-10 week old kitten, slender, NOT adult, NOT chubby, NOT photorealistic, NOT a real cat), big round Pixar-style eyes — each eye has a LARGE PURE WHITE sclera (white of the eye clearly visible all around) surrounding a medium round EMERALD-GREEN iris (#3DDC84) with a black pupil; ONLY the small iris is green, the white of the eye stays pure white, NEVER a fully-green eyeball. Small thin round gold-framed glasses ALWAYS on. A plain smooth brown leather collar (smooth band, NO studs, NO buckle) with EXACTLY ONE clearly HEART-SHAPED gold tag engraved "Brain" (NOT round, NOT a disc) — collar clearly visible. Soft fluffy orange tabby fur, ALL 4 paws — both FRONT and both BACK paws — fully ginger tabby (NO white socks, NO white back paws), tail ginger to the tip, EXACTLY 2 ears, EXACTLY 4 paws, mouth closed, proud dutiful expression. COSTUME: a cute black-and-gold station master's peaked cap on his head — the brown collar with heart tag still visible. SETTING: sitting proudly on a small wooden platform of a charming cartoon Japanese countryside train station, a cute little train and a station sign softly blurred behind, warm daylight. Wholesome. Vertical 9:16.
```
**🎬 Kling (image-to-video):**
```
Brain sits proudly at his station post, gives a small dutiful head-lift, ears perk, the little train softly puffs behind him. Gentle push-in. Preserve the input image exactly — same kitten, same anatomy, glasses, eyes (large white sclera with small emerald iris), collar + heart tag, ginger paws and tail, station master cap costume, Pixar cartoon style. Only animate the motion described — do NOT redraw, restyle, recolor or morph. Mouth closed. Vertical 9:16.
```
**🎙️ VO (EN):**
```
Number three — Tama. This little cat became a train station master in Japan, and saved the whole railway from closing.
```
**🎙️ VO (RU):**
```
Номер три — Тама. Эта маленькая кошка стала начальником ж/д станции в Японии и спасла дорогу от закрытия.
```
**Оверлей (CapCut):** цифра «3» + подпись «TAMA · STATION MASTER» + мягкий «train» SFX.

---

## Сцена 5 — №4 ЛАРРИ (резиденция премьера) — 28–35с
**Что зритель видит:** Brain у знаменитой чёрной двери «№10» с маленькой табличкой «CHIEF MOUSER», гордый. Подпись «LARRY · 10 DOWNING ST». Цифра «4». VO: живёт у премьера, должность — главный мышелов.

**🎨 Image (Nano Banana 2):**
```
Cute Pixar/Disney 3D ANIMATED CARTOON orange tabby kitten named Brain (8-10 week old kitten, slender, NOT adult, NOT chubby, NOT photorealistic, NOT a real cat), big round Pixar-style eyes — each eye has a LARGE PURE WHITE sclera (white of the eye clearly visible all around) surrounding a medium round EMERALD-GREEN iris (#3DDC84) with a black pupil; ONLY the small iris is green, the white of the eye stays pure white, NEVER a fully-green eyeball. Small thin round gold-framed glasses ALWAYS on. A plain smooth brown leather collar (smooth band, NO studs, NO buckle) with EXACTLY ONE clearly HEART-SHAPED gold tag engraved "Brain" (NOT round, NOT a disc) — collar clearly visible. Soft fluffy orange tabby fur, ALL 4 paws — both FRONT and both BACK paws — fully ginger tabby (NO white socks, NO white back paws), tail ginger to the tip, EXACTLY 2 ears, EXACTLY 4 paws, mouth closed, proud official expression. SETTING: Brain sits proudly in front of a famous glossy black government front door with a brass "10" numeral and a brass letterbox (clearly the British Prime Minister's residence look), a small brass plaque beside him. NO costume needed (he's the resident cat). Warm dignified daylight. Vertical 9:16.
```
**🎬 Kling (image-to-video):**
```
Brain sits proudly by the black "10" door, gives a confident little chin-up and a slow blink, tail curling proudly. Gentle push-in. Preserve the input image exactly — same kitten, same anatomy, glasses, eyes (large white sclera with small emerald iris), collar + heart tag, ginger paws and tail, Pixar cartoon style. Only animate the motion described — do NOT redraw, restyle, recolor or morph. Mouth closed. Vertical 9:16.
```
**🎙️ VO (EN):**
```
Number four — Larry. He lives at the British Prime Minister's house with an official job title: Chief Mouser.
```
**🎙️ VO (RU):**
```
Номер четыре — Ларри. Он живёт в доме премьер-министра Британии с официальной должностью: главный мышелов.
```
**Оверлей (CapCut):** цифра «4» + подпись «LARRY · 10 DOWNING ST» + мягкий «posh ding» SFX.

---

## Сцена 6 — №5 СТАББС (кот-мэр) — 35–42с
**Что зритель видит:** Brain в ленте мэра «MAYOR» с маленькой розеткой, важный, на фоне уютного городка. Подпись «STUBBS · CAT MAYOR · 20 YEARS». Цифра «5». VO: 20 лет был почётным мэром на Аляске.

**🎨 Image (Nano Banana 2):**
```
Cute Pixar/Disney 3D ANIMATED CARTOON orange tabby kitten named Brain (8-10 week old kitten, slender, NOT adult, NOT chubby, NOT photorealistic, NOT a real cat), big round Pixar-style eyes — each eye has a LARGE PURE WHITE sclera (white of the eye clearly visible all around) surrounding a medium round EMERALD-GREEN iris (#3DDC84) with a black pupil; ONLY the small iris is green, the white of the eye stays pure white, NEVER a fully-green eyeball. Small thin round gold-framed glasses ALWAYS on. A plain smooth brown leather collar (smooth band, NO studs, NO buckle) with EXACTLY ONE clearly HEART-SHAPED gold tag engraved "Brain" (NOT round, NOT a disc) — collar clearly visible. Soft fluffy orange tabby fur, ALL 4 paws — both FRONT and both BACK paws — fully ginger tabby (NO white socks, NO white back paws), tail ginger to the tip, EXACTLY 2 ears, EXACTLY 4 paws, mouth closed, important proud smile-expression. COSTUME: a cute red-and-gold ceremonial MAYOR sash across his chest with a small gold rosette badge — the brown collar with heart tag still visible above the sash. SETTING: sitting importantly on a small wooden podium in a cozy little cartoon town street with soft mountains behind (Alaskan small-town vibe), warm daylight. Wholesome. Vertical 9:16.
```
**🎬 Kling (image-to-video):**
```
Brain sits importantly on the podium, lifts his chin proudly and gives a tiny dignified nod, the mayor sash catches the light, tail curled neatly. Gentle push-in. Preserve the input image exactly — same kitten, same anatomy, glasses, eyes (large white sclera with small emerald iris), collar + heart tag, ginger paws and tail, mayor sash costume, Pixar cartoon style. Only animate the motion described — do NOT redraw, restyle, recolor or morph. Mouth closed. Vertical 9:16.
```
**🎙️ VO (EN):**
```
Number five — Stubbs. He was the honorary mayor of a town in Alaska for twenty years.
```
**🎙️ VO (RU):**
```
Номер пять — Стаббс. Он двадцать лет был почётным мэром городка на Аляске.
```
**Оверлей (CapCut):** цифра «5» + подпись «STUBBS · CAT MAYOR · 20 YEARS» + мягкий «applause» SFX.

---

## Сцена 7 — ТЁПЛЫЙ ТВИСТ (host) — 42–49с
**Что зритель видит:** Brain снова в гостиной, тёплый гордый взгляд, вокруг мягко парят 5 маленьких звёздочек/иконок прошедших котов, сердечко. Подпись «CATS CHANGE THE WORLD». VO: кот может изменить мир, просто оставаясь собой.

**🎨 Image (Nano Banana 2):**
```
Cute Pixar/Disney 3D ANIMATED CARTOON orange tabby kitten named Brain (8-10 week old kitten, slender, NOT adult, NOT chubby, NOT photorealistic, NOT a real cat), big round Pixar-style eyes — each eye has a LARGE PURE WHITE sclera (white of the eye clearly visible all around) surrounding a medium round EMERALD-GREEN iris (#3DDC84) with a black pupil; ONLY the small iris is green, the white of the eye stays pure white, NEVER a fully-green eyeball. Small thin round gold-framed glasses ALWAYS on. A plain smooth brown leather collar (smooth band, NO studs, NO buckle) with EXACTLY ONE clearly HEART-SHAPED gold tag engraved "Brain" (NOT round, NOT a disc). Soft fluffy orange tabby fur, ALL 4 paws — both FRONT and both BACK paws — fully ginger tabby (NO white socks, NO white back paws), tail ginger to the tip, EXACTLY 2 ears, EXACTLY 4 paws, mouth closed in a warm proud content expression. INT. COZY LIVING ROOM — DAY (same locked room: cream walls, honey-amber wood floor, ONE plain SOLID sage-green woven rug — no pattern, no border, no fringe — cream armchair, tall window with sheer curtains, snake plant; warm cozy glow). Medium close-up. Brain sits warmly looking at camera, content and proud. Softly floating around him: five small gold stars and tiny icons (rocket, sailor cap, station cap, a "10" door, a mayor sash) representing the five cats, plus one soft pastel-pink heart. Vertical 9:16.
```
**🎬 Kling (image-to-video):**
```
Brain gives a warm proud slow blink to camera, the five little star-icons gently orbit around him and the pink heart floats up softly. Gentle warm push-in. Preserve the input image exactly — same kitten, same anatomy, glasses, eyes (large white sclera with small emerald iris), collar + heart tag, ginger paws and tail, Pixar cartoon style. Only animate the motion described — do NOT redraw, restyle, recolor or morph. Mouth closed. Vertical 9:16.
```
**🎙️ VO (EN):**
```
These cats prove one thing — a cat can change the world just by being itself.
```
**🎙️ VO (RU):**
```
Эти коты доказали одно — кот может изменить мир, просто оставаясь собой.
```
**Оверлей (CapCut):** «CATS CHANGE THE WORLD» в нижней трети + тёплый music-resolve.

---

## Сцена 8 — CTA (open-loop) — 49–56с
**🎬 Что в кадре:** универсальный клип `assets/universal-scene-8.mp4` (Brain машет + slow-blink). ИЛИ короткий Kling: Brain кивает на пустой диван с лёгкой ухмылкой.
**🎙️ VO (EN):**
```
Subscribe — maybe the next famous cat is sleeping on your couch right now.
```
**🎙️ VO (RU):**
```
Подпишись — может, следующий знаменитый кот спит на твоём диване прямо сейчас.
```
> Интригующий open-loop CTA (фикс из killer-разбора).

---

## 🎵 Музыка (Suno)
```
Warm adventurous "hall of fame" cat-facts score for a 56-second YouTube Short "Famous Cats Who Made History". Open 0-7s bright curious heroic sting (light orchestral + twinkle). Playful adventurous build 7-42s, each beat slightly different flavor (a touch of space shimmer, a sea-breeze lilt, a gentle Japanese koto note, a posh string flourish, a triumphant little fanfare) but cohesive. Warm proud major-key swell 42-49s for the "cats change the world" payoff. Soft inviting close 49-56s. 100 BPM, no vocals, mood: heroic-wholesome → warm. Royalty-free.
```

## Сборка / монтаж
- VO -12 LUFS, музыка -18 LUFS, субтитры верхняя треть ≤4 слова
- Цифры «1-5» + подписи имён — в CapCut, шрифт залочен
- Хард-cut сцен; host (1,7) в гостиной, виньетки (2-6) в своих мини-сценах
- Виньетки держать ПРОСТЫМИ (один яркий реквизит + чистый фон) — меньше дрифта и читаемо на телефоне
- Тон wholesome, без опасности (Сэм — без тонущих кораблей; космос — дружелюбный)

## 📌 Pinned comment (человеческий тон + ask на стендалоны)
🇬🇧:
```
honestly the space cat broke my brain, i had no idea 😭 i wanna do a full video on one of these — which cat do you wanna know everything about?
```
🇷🇺:
```
кот в космосе реально сломал мне мозг, я вообще не знал 😭 хочу сделать целое видео про одного из них — про кого рассказать подробно?
```
> Этот закреп = деманд-сигнал: аудитория голосует, какой стендалон делать первым (Фелисетт / Ларри / Стаббс / Сэм).

## SEO Pack

**Title:** `A Cat Went to SPACE?! 5 Famous Cats Who Made History 🐱 | Cat Facts`

**Alt titles (A/B):**
```
5 Famous Cats Who Made History 🐱 | Cat Facts
These 5 Cats Are More Famous Than You 🐱 | Cat Facts
A Cat in Space, a Cat Mayor & 3 More Legends 🐱 | Cat Facts
```

**Russian titles (для RU-зеркала, НЕ для EN-канала):**
```
Кот Летал в КОСМОС?! 5 Знаменитых Котов в Истории 🐱
5 Котов, Которые Изменили Историю 🐱
```

**Description (≥250 символов, ключ 2-3×):**
```
Some cats are more famous than movie stars — and their real stories are wild. 🐱 In this cat facts short: 5 famous cats who made history. A cat who flew to space (Félicette, 1963), a cat who survived three shipwrecks (Unsinkable Sam), a cat who became a Japanese train station master and saved the line (Tama), the Chief Mouser who lives with the British Prime Minister (Larry), and a cat who was mayor of an Alaskan town for 20 years (Stubbs).

These famous cats prove a cat can change the world just by being itself. 💚

🐱 Follow Brain for cat facts, cat history and cat behavior every day.

#shorts #catfacts #famouscats #catfactsmindblowing #braincatai #cathistory #catpsychology
```

**Tags (base + video-specific, ~500 символов):**
```
famous cats, top 10 cats, famous cats in history, most famous cats, cat facts, cat history, cats who made history, felicette space cat, first cat in space, unsinkable sam, tama station master cat, larry chief mouser, stubbs cat mayor, famous internet cats, brain cat, braincatai, cat facts mind blowing, cat facts daily, animal facts, mind blowing facts, did you know cats, legendary cats, cats in history, amazing cats, cat stories
```

**Hashtags (тело описания):**
```
#shorts #catfacts #famouscats #catfactsmindblowing #braincatai #cathistory #catpsychology #petfacts
```

---

## 🖼️ ТУМБНЕЙЛ (Nano Banana 2 — «5 Signs» формула: лицо 60%, dilated глаза, hook-объект, крупный поднятый текст)
```
EXTREME CLOSE-UP of cute Pixar/Disney 3D ANIMATED CARTOON orange tabby kitten Brain — face fills ~60% of the LEFT-CENTER of frame, slight 3° tilt. Big round Pixar-style eyes — each eye has a LARGE PURE WHITE sclera (white of the eye clearly visible all around) surrounding a medium round EMERALD-GREEN iris (#3DDC84) with a black pupil; ONLY the small iris is green, the white of the eye stays pure white, NEVER a fully-green eyeball. EYES WIDE AND DILATED with a SHOCKED-AMAZED "WHAT?!" look, locked into the viewer, eyebrows up. Ears perked sharply UP. Mouth closed in a small surprised "o". Small thin round gold-framed glasses. On his head: a cute cartoon astronaut helmet with a clear visor pushed up (signals "cat in space"). Plain brown leather collar with EXACTLY ONE clearly HEART-SHAPED gold tag engraved "Brain". Soft fluffy orange tabby fur, ALL 4 paws ginger (front and back, no white socks), tail ginger to the tip, EXACTLY 2 ears. Pixar/Disney 3D ANIMATED CARTOON, NOT photorealistic, NOT a real cat. Vertical 9:16.

HOOK ELEMENT (lower-RIGHT, ~22% area): a small cartoon rocket + a few gold stars / a tiny planet (signals "space") on a deep-blue starry corner.

BACKGROUND: warm cozy bokeh blending into a soft starry-blue glow, HEAVILY BLURRED, heavy DOF. Brain rim-lit so he POPS. High contrast.

TEXT PLATE (BIG and BOLD, positioned in the LOWER-MIDDLE of the frame at roughly 55-70% height — raised CLEAR of the very bottom edge so it is NOT cut off by the phone UI / duration badge. LARGE, filling ~85-90% of frame width, easily readable on a small phone. 2 lines, ALL CAPS, slight 2-3° tilt, BOLD ROUNDED GEOMETRIC SANS-SERIF Fredoka One / Nunito Bold, Electric Yellow #FFD23F fill, solid charcoal #2B2B2B outline 6px, soft drop-shadow). Lines:
LINE 1: "A CAT WENT"
LINE 2: "TO SPACE?! 🚀"

TYPOGRAPHY LOCK (strict): single rounded sans-serif font, NO serifs, NO Comic Sans, NO Papyrus, NO Impact.
EYE COLOR RULE (strict): LARGE PURE WHITE sclera + medium EMERALD-GREEN iris (#3DDC84) + black pupil; only the small iris is green, sclera stays white, NEVER a fully-green eyeball.
ANATOMY RULE (strict): EXACTLY 4 paws AND EXACTLY 2 EARS.
STYLE RULE (strict): Pixar/Disney 3D ANIMATED CARTOON. NOT photorealistic.
LANGUAGE RULE (strict): on-screen text ENGLISH only. NO Russian, NO Cyrillic.
```
**Thumbnail negatives:**
```
2D flat, anime, photorealistic, real cat, photoreal, hyperrealistic, multiple cats, second cat, low quality, blurry, cluttered background, calm boring face, low contrast, blue eyes, amber eyes, brown eyes, green sclera, green eye-whites, fully green eyes, white paws, white back paws, round tag, bell tag, blank tag, missing glasses, mouth open as if talking, scary, gore, serif font, Comic Sans, Papyrus, Impact, gradient text, neon text, multiple fonts, Russian text, Cyrillic letters, watermark, logo
```
> A/B-альтернативы для текста: `5 LEGENDARY CATS 🤯` / `A CAT MAYOR?! 😼` / `FAMOUS CATS 🚀`

---

## 🛑 Pre-flight
- Все image-промты = Nano Banana 2, с полным Locked Brain (анатомия глаза, рыжие задние лапы, сердечко-кулон без слова "holes")
- Kling = image-to-video, motion-only + preserve-suffix
- **Костюмы НЕ закрывают ошейник** — сердечко «Brain» видно у выреза в каждой виньетке
- Один Brain в кадре везде (никаких реальных/вторых котов) — негатив `second cat`
- Цифры «1-5» + подписи имён — CapCut, залоченный шрифт
- Виньетки простые (один реквизит + чистый фон) — меньше дрифта
- Тон wholesome: космос дружелюбный, корабль БЕЗ тонущих сцен (негатив добавить при необходимости)

## Метрики (сравнить с killer / TV / circus)
| | 48ч | 7д |
|--|--|--|
| CTR (цель >5%) | | |
| Retention (цель >50%) | | |
| Swipe-stay | | |
| Подписки | | |
