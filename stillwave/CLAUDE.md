...
- **🔒 Reserve the lower-left for text WHEN GENERATING the image.** Every NanoBanana / image-generation prompt MUST compose the scene so the **lower-left corner is clean negative space** — a dark, uncluttered, low-detail area (shadowed floor, plain wall, dim foreground) with NO key subject, bright light source, or busy detail there. Put the hero/subject and bright elements on the RIGHT or CENTER, lean the dark/empty space to the lower-left. This guarantees the lower-left text overlay always lands on a readable dark zone. Add a line to every image prompt like: *"keep the lower-left corner as dark, empty negative space for a text overlay — no bright objects or busy detail in the bottom-left third."*
