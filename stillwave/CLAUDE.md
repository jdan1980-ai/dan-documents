...
- **🔒 Reserve the lower-RIGHT corner for the channel LOGO — EVERY generation (LOCKED 2026-07-16).** The StillWave logo is placed in the bottom-right corner in post — it covers the generator's watermark. Therefore EVERY NanoBanana / image prompt (16:9 AND 9:16) must keep the **bottom-right corner as calm, dark, low-detail space** — no key subject, no figure, no bright light source, no busy detail there. Add to every prompt a line like: *"keep the bottom-right corner as calm, dark, low-detail space — no subject or bright detail there (logo placement)."* Corners map for 16:9: **lower-left = text overlay · lower-right = logo.**
- **🔒 Reserve the lower-left for text WHEN GENERATING the image — 16:9 ONLY (NOT 9:16).** Every NanoBanana / image-generation prompt for a **16:9** thumbnail/scene MUST compose so the **lower-left corner is clean negative space** — a dark, uncluttered, low-detail area (shadowed floor, plain wall, dim foreground) with NO key subject, bright light source, or busy detail there. Put the hero/subject and bright elements on the RIGHT or CENTER, lean the dark/empty space to the lower-left, so the lower-left text overlay always lands on a readable dark zone. Add a line to every 16:9 prompt like: *"keep the lower-left corner as dark, empty negative space for a text overlay — no bright objects or busy detail in the bottom-left third."* **Do NOT apply this to 9:16 Shorts** — vertical framing composes differently, leave it unconstrained.
