...
`stillwave/assets/garyu-compose-thumb.py <hero.jpg>` делает и превью, и оверлей, и печатает проверочные числа. Кроп подобран замером, а не на глаз:

| кроп | шлем в кадре | в ленте 168px | что в кадре |
|---|---|---|---|
| весь кадр | 38% | 64px | оскал — тёмное пятно ❌ |
| x500 w1894 | 48% | 81px | вся сцена, маска всё ещё не читается ❌ |
| **x900 w1494** | **61%** | **102px** | **фигура + дым + маска — взято** ✅ |
| x1050 w1344 | 68% | 114px | маска лучше всех, но человека нет ❌ |

Самый тесный кроп читается лучше всего, но выбрасывает фигуру спиной к камере — а это проверенный сигнал шаблона Kanji-Concept (KAMI, TORA, BUDŌ). Средний держит оба: маска читается, фигура и хвост остаются, дымный дракон идёт силуэтом по золоту.

Числа готового файла: текст занимает `x 4.5–27.7%`, `y 14.2–90.4%`; правый нижний угол под логотип — яркость **40** (порог 60). 臥龍 идёт золотом с размытым тёмным ореолом, иначе золото по золоту тонет.

**🇺🇦** Кроп выбран по замеру, а не на глаз: в полном кадре шлем 38% ширины и в ленте оскал не читается; тесный кроп читается лучше всех, но убирает фигуру — берём средний, он держит и маску, и человека.
