# WABI SABI — 侘寂 | 2H Japanese Zen Music

## Meta

- **Title:** WABI SABI — 侘寂 | 2H Japanese Zen Music for Letting Go, Imperfection & Deep Calm
- **Slug:** `wabi-sabi-2h`
- **Format:** Long-form (Kanji-Concept Series)
- **Length:** 2:00:39 — FINAL (36 mastered tracks; 41 generated, 5 dropped). Conscious 39-sec overage past the 2H cap — reads as an even "2 hours" on YouTube; user call 2026-07-17.
- **Aesthetic:** Kintsugi bowl still life — dark tearoom, a broken Japanese chawan mended with gold seams, one soft side light. The gold cracks ARE the visual hero.
- **Playlist (add to in Studio):** Japanese Zen Music
- **Status:** 📅 SCHEDULED — long-form publishes 2026-07-18 18:00 + Community Post same day (moved up one day to ride the MUSHIN wave; 36 tracks / 2:00:39 / thumbnail = kintsugi bowl + gold kanji). Short (§14) uploaded, scheduled 2026-07-19 18:00 — day-after funnel to the long-form.
- **Concept note:** WABI-SABI (侘寂) — beauty of imperfection and impermanence. Kintsugi (金継ぎ, golden repair) is its most photogenic embodiment: the break is not hidden, it is honored. Strong crossover appeal (self-help / healing / acceptance audience + zen music cluster). Concept marked free-for-use in the Kanji-Concept bank.

---

## §1 — Suno Style variants

Three style variants — rotate across tracklist to avoid Content ID on openings (SATORI lesson).
Batch naming for Suno: `WABISABI` = A · `WABISABI 2` = B · `WABISABI 3` = C. Interleave in final lay-down (no two adjacent tracks share an opening) — same as-built process as IKIGAI.

> **✅ AS BUILT (2026-07-17):** generated 41 tracks in 3 batches (`WABI SABI 1` = A ×18 · `WABI SABI 2` = B ×14 · `WABI SABI 3` = C ×9). Per user priority ALL 18 A-tracks kept; dropped 5: `2 (3)`, `2 (12)`, `2 (13)`, `3 (4)`, `3 (7)` — parked in `…-mastered/extra/`. `WABI SABI 2 (1)` (TP 0.0 in Suno — mastered file peak-safe, verify by ear) restored per user as the closing epilogue track #36 "The Shelf in Shadow". Final order = strict A-alternation through #35, then B epilogue → no two adjacent tracks share an opening. Files renamed `01…36 <poetic name>.wav`, filename sort = CapCut order.

### Variant A — Style field (Pad-led stillness)

```
Slow Japanese ambient meditation, deep warm drone pad foundation, sparse shakuhachi flute phrases, occasional koto notes like drops of gold, distant temple bell, soft tape-warm texture, 50-58 BPM, intimate tearoom atmosphere, candlelight stillness, instrumental only, no vocals, no singing, no chanting, no spoken word, pure instrumental
```

### Variant B — Style field (Rain-led acceptance)

```
Quiet Japanese zen ambient, gentle rain on tearoom roof opening, low shakuhachi breath tones, soft koto arpeggios, warm sub pad, subtle singing bowl shimmer, 50-58 BPM, rainy evening in an old tea house, aged wood and clay, instrumental only, no vocals, no singing, no chanting, no spoken word, pure instrumental
```

### Variant C — Style field (Bell-led impermanence)

```
Meditative Japanese ambient, single bonsho temple bell opening with long decay, hushed shakuhachi melody, sparse koto, deep still pad, faint crackle of a charcoal brazier, 50-58 BPM, twilight temple stillness, everything passing and at peace, instrumental only, no vocals, no singing, no chanting, no spoken word, pure instrumental
```

---

## §2 — Suno Lyrics field variants

### Variant A — Lyrics (Pad-led)

```
[no lyrics, no vocals, instrumental only]
[opening: slow swelling warm drone pad alone for the first 6-8 seconds, no melodic instrument until 0:08]
[section A: shakuhachi enters like a slow exhale over the pad, long silences between phrases]
[section B: koto notes fall one by one, unhurried, like lacquer drops hardening into gold]
[section C: pad deepens, flute and koto trade single phrases, distant bell once]
[loop point: pad sustains and settles back to the opening drone]
[mood: a cracked tea bowl on a dark shelf, gold seams catching one candle's light]
[texture: warm analog drift, soft room air, aged wood]
```

### Variant B — Lyrics (Rain-led)

```
[no lyrics, no vocals, instrumental only]
[opening: 4 seconds of gentle rain on a tearoom roof, then shakuhachi enters softly around 0:04]
[section A: rain settles under sparse flute phrases, koto answers quietly]
[section B: warm pad rises beneath, rain becomes texture not subject]
[section C: koto arpeggio slows to single notes, flute holds one long tone]
[loop point: rain alone for two bars, ready to begin again]
[mood: an old potter watching rain, broken bowls waiting patiently on the bench]
[texture: soft rain grain, clay and wood, candle warmth]
```

### Variant C — Lyrics (Bell-led)

```
[no lyrics, no vocals, instrumental only]
[opening: single low temple bell strike, 4-5 seconds of resonance, then shakuhachi enters quietly]
[section A: flute phrases short and humble, wide silences, pad barely breathing underneath]
[section B: koto enters with a falling three-note figure, repeated like a quiet acceptance]
[section C: bell once more, far away; instruments thin out toward stillness]
[loop point: the bell's decay fades into the opening silence]
[mood: twilight in an empty temple, one imperfect bowl on the altar, everything passing]
[texture: long decays, air and space, charcoal warmth]
```

---

## §3 — NanoBanana 16:9 prompt (thumbnail + video visual)

**PRIMARY (locked 2026-07-16) — the master at work.** Elderly craftsman mending the bowl, over-the-shoulder, face never visible (channel lock), hands FROZEN mid-gesture (brush poised, not moving — loop-safe). Historically he is "a master craftsman", NOT named Sen no Rikyū in any public copy (Rikyū was a tea master, not a potter/lacquerer — his name stays in the §12 Community Post quote only).

```
Photorealistic cinematic, Kurosawa film quality, 4K, wide 16:9 shot. An elderly Japanese master craftsman in a dark indigo work kimono and headcloth, seen from behind over his shoulder, face not visible, kneeling at the CENTER of the frame at a low wooden workbench in a shadowed tearoom workshop. In his weathered hands he holds a broken Japanese tea bowl, a fine brush poised motionless over a crack filled with luminous gold lacquer — the glowing kintsugi seam is the single bright element. Soft daylight from a shoji window in the upper right rakes across his hands and the bowl. The workbench with small pots of urushi lacquer and gold powder sits to his left; dark wooden shelves with old bowls fade into shadow on the far left. The FOREGROUND is empty dark tatami floor stretching across the entire bottom of the frame — both the bottom-left and bottom-right corners are calm, dark, empty negative space with no objects, no bright details, no parts of the figure (bottom-left = text overlay, bottom-right = channel logo). The master's body stays fully inside the middle third of the frame, never touching the left or right edge. Deep charcoal and umber palette, ONE warm gold accent. Reverent, museum-like stillness. No text, no logos, no watermarks.
```

*(Composition v2 locked 2026-07-16: master centered, BOTH bottom corners clean — left for the wisdom overlay, right for the channel logo that covers the generator watermark.)*

**Alt A — still life (no figure, original concept):**

```
Photorealistic cinematic still life, Kurosawa film quality, 4K. A single broken Japanese tea bowl (chawan) mended in the kintsugi tradition — its cracks filled with luminous gold lacquer seams — standing on a dark aged wooden shelf in a shadowed tearoom. One soft warm side light from the right rakes across the bowl, making the gold veins glow against the deep charcoal-black background; the rough clay surface shows its texture and small chips honestly. Behind the bowl, darkness fades to black with the faintest suggestion of tearoom timbers. Deep charcoal and umber palette with ONE warm gold accent — the shining kintsugi seams. Quiet, reverent, museum-like stillness. Keep the lower-left corner as dark, empty negative space for a text overlay — no bright objects or busy detail in the bottom-left third. No text, no logos, no watermarks.
```

**Alt B — hands macro (also Shorts frame 2 option):**

```
Photorealistic cinematic close-up, Kurosawa film quality, 4K. The weathered hands of an elderly Japanese craftsman cradling a broken tea bowl, a fine brush held motionless as liquid gold lacquer fills the crack — the kintsugi seam glowing warmly in soft side light, every wrinkle of the hands and grain of the rough clay in sharp detail. Deep charcoal darkness around; small dish of gold powder blurred at the frame edge. No face visible. ONE warm gold accent in a dark umber palette. Keep the lower-left corner as dark, empty negative space for a text overlay. No text, no logos, no watermarks.
```

---

## §3c — Closing scene: tea ceremony with the mended bowl (finale, locked 2026-07-16)

The narrative arc: broken → mended with gold → SERVES TEA AGAIN. Use as the video's alternate visual chapter and the Shorts finale.

**Variant A — whisking matcha in the kintsugi bowl (recommended):**

```
Photorealistic cinematic close-up, Kurosawa film quality, 4K, 16:9. A Japanese tea ceremony moment: weathered hands holding a bamboo chasen whisk poised motionless over the kintsugi tea bowl — the same dark chawan with luminous gold seams — filled with vivid green matcha, a thin wisp of steam rising from the hot tea. Soft warm side light in a shadowed tearoom, tatami and a dark iron kettle blurred in the background. No face visible. Deep charcoal palette, the glowing gold seams and the green matcha as the only color accents. Keep the lower-left corner as dark, empty negative space for a text overlay, and keep the bottom-right corner as calm, dark, low-detail space — no subject or bright detail there (logo placement). Quiet, ceremonial stillness. No text, no logos, no watermarks.
```

**Variant B — after the ceremony, no people:**

```
Photorealistic cinematic still life, Kurosawa film quality, 4K, 16:9. After the tea ceremony: the kintsugi bowl with glowing gold seams stands on a tatami mat, half full of green matcha, a faint wisp of steam rising. Beside it a bamboo chasen whisk resting on a folded cloth and a dark iron kettle in soft shadow. Warm late-afternoon light from a shoji window rakes low across the tatami weave. No people. Deep charcoal and umber palette, gold seams and matcha green as the only accents. Keep the lower-left corner as dark, empty negative space for a text overlay, and keep the bottom-right corner as calm, dark, low-detail space — no subject or bright detail there (logo placement). No text, no logos, no watermarks.
```

**Flow loop (Variant A):**

```
Camera completely locked. Animate only three subtle elements: (1) the thin wisp of steam rising slowly from the hot matcha, gently swaying, (2) the gold seams glinting softly with the breathing light, (3) the warm light itself brightening and dimming almost imperceptibly. The hands, the whisk, the bowl, everything else — completely motionless. Seamless loop.
```

*(Steam is safe HERE — it rises from the hot tea intentionally. This is the one kintsugi scene where steam/smoke is allowed.)*

**Bowl-in-void turntable (Shorts, generated 2026-07-16 — bowl centered in black space):**

```
Camera completely locked, no pan, no zoom. The kintsugi bowl rotates slowly and steadily on its axis like a museum turntable — completing exactly ONE full 360-degree rotation over the clip, constant speed, ending in the same position it started. As it turns, the gold seams radiate a soft inner glow that gently pulses — brightening and breathing like embers, casting a faint warm reflection on the dark surface beneath. The background stays pure black and completely still. The bowl keeps its exact shape, size, texture and crack pattern throughout the rotation. Smooth, hypnotic, reverent.
```

---

## §3b — NanoBanana 9:16 prompt (Shorts)

```
Vertical 9:16 photorealistic cinematic still life, Kurosawa film quality, 4K. A single broken Japanese tea bowl (chawan) mended in the kintsugi tradition — cracks filled with luminous gold lacquer — standing on a dark aged wooden shelf, deep charcoal-black tearoom darkness above and below. One soft warm side light from the right makes the gold seams glow; rough clay texture and honest small chips visible. The bowl sits in the lower half of the frame; the upper half is calm dark negative space with the faintest warm light gradient. Deep charcoal and umber palette, ONE warm gold accent. No text, no logos, no watermarks.
```

---

## §4 — Flow / Kling loop prompt (8-sec seamless)

**For the PRIMARY master-at-work image (v2 — NO smoke, lesson 2026-07-16):**

```
Camera completely locked, no pan, no zoom. Animate only three subtle elements: (1) the warm side light breathes very slowly, as if by candlelight, (2) the gold lacquer seam on the bowl glints and softens with the breathing light, (3) the soft light gradient on the dark back wall shifts almost imperceptibly. The master, his hands, the brush, the bowl, the workbench — completely motionless, a held breath. Seamless loop, last frame matches first frame.
```

**For the Alt A still life (v2 — NO smoke):**

```
Camera completely locked, no pan, no zoom. Animate only three subtle elements: (1) the warm side light breathes very slowly — the gold kintsugi seams glint and soften as if by candlelight, (2) the shadows on the shelf deepen and release with the breathing light, (3) the soft light gradient on the back wall shifts almost imperceptibly. The bowl itself, the shelf, everything else — completely motionless. Seamless loop, last frame matches first frame.
```

*(No airborne particles — no dust motes, no petals. ⚠️ **NO SMOKE in kintsugi scenes** — lesson 2026-07-16: any mention of incense smoke makes the model attach the smoke to the bowl/brush, reading as if the bowl is smoking. Same principle as the NO-negative-mention rule: just omit it. Light-breathing + gold-glint + wall-gradient are the safe trio. ⚠️ The master's hands must be a HELD pose — any hand motion in an 8-sec loop reads as a glitch repeated 900×.)*

---

## §5 — Tracklist (FINAL — real timestamps from mastered files, 2026-07-17)

36 tracks · 2:00:39 total · A-tracks on every odd position through #35, B epilogue closes (user priority: all of batch 1 kept). Arc: the break → the mending → gold in the cracks → acceptance → the shelf.
*(4 names unused after the trim: "The Chip on the Rim", "The Candle and the Bowl", "The Kiln Remembers", "Faded Indigo".)*

```
0:00 The Bowl That Fell
3:25 What Breaking Teaches
6:40 Shards on the Workbench
10:04 The Potter Does Not Weep
13:34 Lacquer and Patience
17:12 The First Golden Seam
20:22 Nothing Is Finished
23:51 Nothing Is Perfect
27:10 Nothing Lasts
31:09 The Beauty of the Flaw
34:14 Where the Light Enters
37:54 Gold in the Cracks
40:53 The Repair Is the Story
44:08 An Honest Surface
47:18 Tea Tastes Deeper Now
50:22 Rain on the Tearoom Roof
53:22 Old Wood, Older Clay
56:37 The Master's Favorite Cup
59:56 A Scar Worth Keeping
1:03:25 Twilight in the Workshop
1:06:32 Weathered, Not Worn Out
1:09:31 The Moss Chooses the Stone
1:13:00 The Season of Falling
1:16:19 What Remains Is Enough
1:19:20 The Unswept Garden
1:22:40 Asymmetry at Rest
1:25:49 The Silence Between Notes
1:28:57 Everything Passing
1:32:06 A Bowl Full of Evening
1:35:36 The Glaze Crackles Softly
1:38:44 Patina of Years
1:41:54 Held Together by Light
1:45:16 The Last Firing
1:48:40 Beauty Keeps No Schedule
1:51:44 Whole Again, Differently
1:57:10 The Shelf in Shadow
```

---

## §6a — Wisdom Overlay

- **Phrase:** 諸行無常
- **Romaji:** Shogyō mujō
- **Gloss:** "All things are impermanent"
- **Context:** The opening words of the Heike Monogatari and a core Buddhist yojijukugo — the doctrinal heart of wabi-sabi: nothing lasts, nothing is finished, nothing is perfect.
- **Position:** Lower-left — over the dark empty negative space beside the shelf
- **Clip start:** 0:03 | **Clip end:** 0:14 | Fade In 2s + Fade Out 2s
- **Font:** Liberation Serif Bold, cream #F5EAD2
- **CapCut:** TEXT layer on TOP track. Start 0:03, end 0:14. Fade In 2s + Fade Out 2s. No glow, no shadow, no box.

---

## §6b — CapCut Assembly notes

1. Video track: Flow loop repeated to cover ~2H
2. Audio track 1: mastered tracks `01…NN` laid end-to-end in filename order (openings differ per variant — no cross-fade needed)
3. Audio track 2 (optional): very soft rain/room-air ambience at –14 dB under the music
4. Text layer: Wisdom overlay §6a — start 0:03, end 0:14
5. Export: 1920×1080, H.264, AAC 192k

---

## §7 — YouTube Title

```
WABI SABI — 侘寂 | 2H Japanese Zen Music for Letting Go, Imperfection & Deep Calm
```

*(~80 chars · "Zen Music" in first 40% ✓ · no hashtags ✓)*

**A/B variant:**
```
WABI SABI — 侘寂 | 2H Japanese Zen Music for Broken Things Made Beautiful
```

*(Score both via VidIQ before upload — same process as IKIGAI title test.)*

---

## §8 — YouTube Description

```
japanese zen music, wabi sabi, kintsugi, japanese ambient music, meditation music, zen music, relaxing music, sleep music, healing music, background music, letting go meditation, acceptance meditation, shakuhachi music, koto music, japanese meditation music, mindfulness music, calming music, peaceful music, japanese philosophy, beauty of imperfection — a broken Japanese tea bowl mended with gold, one candle's light in a dark tearoom.

🌀 WABI SABI means "beauty in imperfection."
Nothing lasts. Nothing is finished.
Nothing is perfect. And that is enough.

The bowl fell, and was not thrown away.
Its cracks were filled with gold —
the break made part of the story,
the scar made the most beautiful line.
What is worn, weathered, and mended
holds more warmth than what is new.

This is WABI SABI — 侘寂.

─────────────────────────────

🎵 Tracklist

0:00 The Bowl That Fell
3:25 What Breaking Teaches
6:40 Shards on the Workbench
10:04 The Potter Does Not Weep
13:34 Lacquer and Patience
17:12 The First Golden Seam
20:22 Nothing Is Finished
23:51 Nothing Is Perfect
27:10 Nothing Lasts
31:09 The Beauty of the Flaw
34:14 Where the Light Enters
37:54 Gold in the Cracks
40:53 The Repair Is the Story
44:08 An Honest Surface
47:18 Tea Tastes Deeper Now
50:22 Rain on the Tearoom Roof
53:22 Old Wood, Older Clay
56:37 The Master's Favorite Cup
59:56 A Scar Worth Keeping
1:03:25 Twilight in the Workshop
1:06:32 Weathered, Not Worn Out
1:09:31 The Moss Chooses the Stone
1:13:00 The Season of Falling
1:16:19 What Remains Is Enough
1:19:20 The Unswept Garden
1:22:40 Asymmetry at Rest
1:25:49 The Silence Between Notes
1:28:57 Everything Passing
1:32:06 A Bowl Full of Evening
1:35:36 The Glaze Crackles Softly
1:38:44 Patina of Years
1:41:54 Held Together by Light
1:45:16 The Last Firing
1:48:40 Beauty Keeps No Schedule
1:51:44 Whole Again, Differently
1:57:10 The Shelf in Shadow

─────────────────────────────

🌀 Let the crack fill with gold.
🍃 Let what is imperfect be at peace.

Subscribe for more Japanese ambient meditation journeys 🌿
```

---

## §9 — Tags (verify broad scores in VidIQ before upload)

```
zen music, meditation music, relaxing music, sleep music, background music, healing music, japanese zen music, japanese ambient music, calming music, peaceful music, shakuhachi music, koto music, flute music, wabi sabi, kintsugi, wabi sabi philosophy, beauty of imperfection, letting go meditation, japanese meditation music, zen meditation music, mindfulness music, japanese philosophy, broken japanese tea bowl mended with gold kintsugi seams glowing in dark tearoom candlelight, joe hisaishi, stillwave
```

*(25 tags · broad-first ✓ · instrument ✓ · concept-narrow ✓ · scene-description + celeb-anchor ✓ · brand 1 ✓)*

---

## §10 — Hashtags

```
#wabisabi #kintsugi #japanesezenmusic #zenmusic #meditationmusic
```

---

## §11 — Pinned Comment

```
🌀 WABI SABI (侘寂) — the beauty of imperfection.

The Japanese don't hide the crack.
They fill it with gold.

What's one "crack" in your life that turned out to be gold? Share below 👇

If this session helped — subscribe. New journeys every week 🌿
```

---

## §12 — Community Post (Kanji-Concept — publish day-of or day-before)

```
WABI SABI (侘寂) in Japanese Culture: A Concise Overview

WABI SABI (侘寂) joins wabi — the beauty of humble simplicity — and sabi — the beauty that time and wear bring. Where Western aesthetics chase symmetry and polish, wabi-sabi finds its deepest beauty in the cracked, the faded, and the incomplete. Its most famous expression is kintsugi (金継ぎ), the art of repairing broken pottery with gold: the break is never hidden but honored as part of the object's history. As the tea master Sen no Rikyū taught, perfection leaves no room for the heart — a garden is swept, and then a few leaves are shaken back onto the moss.
```

---

## §13 — Thumbnail spec

- **Background:** the §3 NanoBanana 16:9 still life — the kintsugi bowl with glowing gold seams IS the click magnet. No compositing.
- **Text 1 — KANJI:** `侘寂` large, upper-center or upper-right over the dark negative space above the bowl — **gold `#E4C46C`** (justified: gold text echoes the kintsugi seams — the entire concept is gold-in-darkness; same logic as the divine-gold exception)
- **Text 2 — ROMAJI:** `WABI SABI` — Liberation Serif Bold, lower-left over the reserved dark negative space, same gold
- No tag/duration text. One font, no outline, no shadow, no glow.
- Compose script: reuse `ikigai-2h-compose-thumb.py` pattern (I render on request when the NanoBanana image is ready).

---

## §14 — Shorts (standalone educational — kintsugi hook)

> **AS-BUILT (2026-07-18):** shipped as the 41-sec / 7-frame version rendered by `stillwave/assets/wabi-sabi-shorts-build.py` (`wabi-sabi-shorts-41s.mp4` — Ken Burns, gold titles TOP, no badge). Uploaded, **scheduled 2026-07-19 18:00** — one day after the long-form. On publish: set Related video → long-form, add to StillWave Shorts playlist, pin the comment below. The 3-frame storyboard beneath is the original concept, kept for reference.

**Concept:** 35-sec educational Short — 3 frames, teaches wabi-sabi through kintsugi, funnels to the long-form.

### Storyboard

| Sec | Frame | Visual | Text overlay |
|-----|-------|--------|--------------|
| 0–4 | 1 HOOK | Broken bowl shards on dark wood | *Why do the Japanese fill cracks with GOLD?* |
| 4–25 | 2 REVEAL | Kintsugi bowl, gold seams glowing (§3b) | **侘寂** + WABI SABI → *Beauty in imperfection* → *The break becomes the most beautiful line* |
| 25–35 | 3 CTA | Extreme close-up of one glowing gold seam | *Full 2-hour session on the channel* + StillWave badge |

### NanoBanana prompts (9:16)

**Frame 1 — HOOK:**
```
Vertical 9:16 photorealistic cinematic still life, Kurosawa film quality, 4K. The shards of a broken Japanese tea bowl lying on dark aged wood in a shadowed tearoom, one soft warm side light raking across the fragments, rough clay edges catching the light. Deep charcoal darkness around them. Quiet, solemn, before-the-repair stillness. No people. No text, no logos, no watermarks.
```

**Frame 2 — REVEAL:** §3b prompt as-is.

**Frame 3 — CTA:**
```
Vertical 9:16 photorealistic cinematic macro shot, Kurosawa film quality, 4K. Extreme close-up of a single kintsugi gold seam running across dark rough ceramic — the luminous gold river of lacquer glowing in warm side light against deep charcoal darkness, clay texture in sharp detail around it. No people. No text, no logos, no watermarks.
```

### Flow / Kling prompts (camera locked)

- **Frame 1:** `Camera completely locked. Animate only the warm side light breathing very slowly across the shards. Everything else completely still. Seamless loop.`
- **Frame 2:** §4 as-is (light breathing + incense ribbon).
- **Frame 3:** `Camera completely locked. Animate only the glow of the gold seam — brightening and softening very subtly as if by candlelight. Everything else completely still. Seamless loop.`

### Shorts Title

```
Why the Japanese Fill Cracks with Gold — WABI SABI 侘寂
```

**A/B variant:** `The Japanese Art of Beautiful Imperfection — WABI SABI 侘寂`

### Shorts Description

```
WABI SABI (侘寂) — the Japanese beauty of imperfection. The break is not hidden. It is honored with gold.

Full 2-hour zen session for letting go and deep calm:
[link after long-form is published]

#wabisabi #kintsugi #zenmusic #shorts
```

### Shorts Tags

```
wabi sabi, kintsugi, japanese philosophy, japanese zen music, beauty of imperfection, golden repair, letting go, japanese culture, zen, meditation music, stillwave
```

### Pinned (Shorts)

```
🌀 WABI SABI — the crack filled with gold. The full 2-hour session is on the channel 🌿
```

### Upload notes

- **Related Video** → the WABI SABI long-form after it's published (primary funnel)
- Playlist: **StillWave Shorts — Japanese Zen & Frequencies**
- Not for kids · first publish Unlisted/Scheduled
