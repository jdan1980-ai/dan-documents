...
> ⚠️ **Ripple attempt failed twice (2026-08-14).** Kling/Flow ties ANY water "ripple" to the monk and generates concentric rings radiating from him — even with explicit "no rings / uniform shimmer" wording. **Fix: freeze the water into a perfect glass mirror; animate ONLY the mist.** A still mirror is on-brand for 和 (perfect symmetry) and rules out rings entirely. Use this prompt:

```
Camera completely locked and static — no pan, no zoom, no push-in, no dolly, no shake. The frame never moves. IMPORTANT: from the very first frame a thin mist is ALREADY present over the water and already drifting — it does not start or appear. The pond is a PERFECTLY STILL, FROZEN GLASS MIRROR: the water surface does NOT move at all — no ripples, no waves, no rings, no circles, no shimmer anywhere — and its reflection is a flawless frozen mirror image that never changes. Animate ONLY these, both extremely subtle: (1) the thin mist over the pond drifts slowly and continuously just above the water; (2) the warm dawn light breathes almost imperceptibly. The monk, the stone, the maple, the lantern, the far bank AND the entire water surface are all completely still and frozen — only the mist drifts. The light does not change colour or overall brightness, does not shift time of day. Slow, warm, balanced, meditative. The mist moves in a smooth continuous cycle so the last frame matches the first frame exactly for a perfectly seamless 8-second loop.
```
**🇺🇦** Рябь не вышла дважды — Kling всегда рисует круги ОТ монаха. Решение: **вода — идеальное замороженное зеркало, двигается только туман.** Никаких колец в принципе, а для 和 неподвижное зеркало даже лучше (идеальная симметрия). Монах, камень, клён, фонарь и вся вода — намертво; живёт только туман над водой. Камера статична, последний кадр = первый.
