#!/usr/bin/env python3
"""UNRYU Short text overlays — transparent PNGs (1080x1920), cream + gold.
...
beat 1 hook (shot 1)    — "The dragon rises only when it finds its clouds" (cream)
beat 2 concept (shot 4) — 雲龍 + UNRYU (gold) over the calm open sky + single peak
beat 3 wisdom (shot 6)  — 雲外蒼天 / Ungai sōten / Beyond the clouds, a blue sky (gold)
"""
...
# ---- beat 1: hook (cream, SERIF) — over fr1 (samurai + dragon, summit) ----
b1 = new()
scrim(b1, 968, 1360)
d = ImageDraw.Draw(b1)
f1 = ImageFont.truetype(SERIF, 70)
for i, line in enumerate(["The dragon rises", "only when it finds", "its own clouds"]):
    d.text((X, 1010 + i * 104), line, font=f1, fill=CREAM)
b1.save(f"{OUT}/unryv_hook.png")

# ---- beat 2: concept 雲龍 + UNRYU — over fr4 (open sky + single distant peak) ----
b2 = new()
...
gold_kanji(b2, ["雲", "龍"], 220, 0, 380, 240, cx0=470)
spaced(b2, "UNRYU", 100, 0, 720, GOLD, ls=20, font=SERIF, cx0=470)
b2.save(f"{OUT}/unryv_concept.png")

# ---- beat 3: wisdom 雲外蒼天 (4 chars) — over fr6 (wide samurai + dragon) ----
b3 = new()
scrim(b3, 1000, 1420, left=-140, right=880)
gold_kanji(b3, ["雲", "外", "蒼", "天"], 118, X, 1036, 128)
spaced(b3, "Ungai soten", 52, X + 4, 1226, GOLD, ls=3, font=SERIF)
ImageDraw.Draw(b3).text((X + 4, 1300), "Beyond the clouds, a blue sky",
                        font=ImageFont.truetype(SERIF, 34), fill=SUB)
b3.save(f"{OUT}/unryv_wisdom.png")

# ---- safe-zone check ----
ok = True
for name in ["unryv_hook", "unryv_concept", "unryv_wisdom"]:
