#!/usr/bin/env python3
"""KARYU wisdom overlay — 1920x1080 transparent PNG for the long-form intro.

不撓不屈 / Futō fukutsu / Indomitable, never yielding — a yojijukugo about a
spirit that does not break under any circumstance. Overlay ≠ title concept
(series rule, same logic as WA/ICHIGO/SUIRYU/UNRYU).

LEFT-lower over the dark foreground stone. In CapCut: TEXT layer on the top
...
OUT = "/home/user/dan-documents/stillwave/assets/karyu-futofukutsu-overlay.png"
...
# soft scrim so cream reads over the dark foreground stone in the lower-left
...
# 不撓不屈 — brush kanji (4 chars), cream, faint warm glow
f = ImageFont.truetype(KANJI, 108)
mask = Image.new("L", (W, H), 0)
md = ImageDraw.Draw(mask)
chars, pitch, y0 = ["不", "撓", "不", "屈"], 128, 692
...
d.text((X + 4, 866), "Futo fukutsu", font=ImageFont.truetype(SERIF, 54), fill=CREAM)
d.text((X + 4, 940), "Indomitable, never yielding",
...
prev.save(f"{SP}/karyu-overlay-preview.jpg", quality=92)
print("preview saved")
