#!/usr/bin/env python3
"""KOMOREBI Short builder — 6 vertical shots, jitter-free Ken Burns (PIL float crop).

Shots (user-generated 9:16 stills, all figure-free):
  fr1 sunbeam forest (hook) · fr2 backlit leaves · fr3 mossy path into light
  fr4 canopy gap / sky (concept 木漏れ日) · fr5 moss-on-stone macro · fr6 golden forest path (wisdom)

Every frame is resampled LANCZOS from a float-precision crop box (never ffmpeg
zoompan); crossfades + overlay alpha fades in PIL; tail fade to black.
"""
...
SHOTS = [
    ("komorebi-shorts-fr1.jpg", True),
    ("komorebi-shorts-fr2.jpg", False),
    ("komorebi-shorts-fr3.jpg", True),
    ("komorebi-shorts-fr4.jpg", False),
    ("komorebi-shorts-fr5.jpg", True),
    ("komorebi-shorts-fr6.jpg", False),
]
ZOOM = 0.10

# step = 5.7, total = 5.7*5 + 6.5 = 35.0s
OVERLAYS = [
    ("kov_hook.png", 3.0, 8.4),           # shot 1
    ("kov_concept.png", 18.0, 23.0),      # shot 4 (starts 17.1)
    ("kov_wisdom.png", 29.6, 34.0),       # shot 6 (starts 28.5)
]
FADE = 0.9
END_FADE = 1.0
