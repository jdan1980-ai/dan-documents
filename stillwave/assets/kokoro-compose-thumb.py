#!/usr/bin/env python3
"""KOKORO thumbnail — 心 upper-centre ABOVE the Buddha + KOKORO low on the monk.

Kanji-Concept canon: KANJI large upper-centre in molten GOLD over the visual hero
(here the golden Buddha triad mural). The Buddha's FACE must stay clear — never
cover a sacred face — so 心 sits high, above the halo, in the faded upper band.
GOLD calligraphic ROMAJI (brush font) LOW, centred on the monk's dark robe
(user pref: romaji down on the foreground, lets the frame breathe). Gold echoes
the gilded mural + candle.
"""
...
def base():
    ...
    im = Image.composite(im.convert("RGBA"), Image.new("RGBA", (W, H), (8, 8, 10, 255)),
...
def sumi(im, chars, size, cx_centre, y, pitch, halo=42):
    ...
    halo_l.paste((6, 6, 9, 255), (0, 0), mask.filter(ImageFilter.GaussianBlur(halo)).point(lambda p: int(p * 0.95)))
