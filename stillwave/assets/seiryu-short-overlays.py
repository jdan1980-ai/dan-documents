#!/usr/bin/env python3
"""SEIRYU Short text overlays — transparent PNGs (1080x1920), cream + gold.
...
beat 1 hook (shot 1)    — "The dark always turns. The light always returns" (cream)
beat 2 concept (shot 4) — 青龍 + SEIRYU (gold) over the calm horizon-at-dawn frame
beat 3 wisdom (shot 6)  — 一陽来復 / Ichiyo raifuku / One yang returns (gold)
"""
...
# ---- beat 1: hook (cream, SERIF) — over fr1 (samurai + starlit dragon) ----
b1 = new()
scrim(b1, 968, 1360)
d = ImageDraw.Draw(b1)
f1 = ImageFont.truetype(SERIF, 62)
for i, line in enumerate(["The dark always turns.", "The light always", "returns."]):
    d.text((X, 1010 + i * 104), line, font=f1, fill=CREAM)
b1.save(f"{OUT}/seiryv_hook.png")

# ---- beat 2: concept 青龍 + SEIRYU — over fr4 (calm horizon at first light) ----
b2 = new()
...
gold_kanji(b2, ["青", "龍"], 220, 0, 380, 240, cx0=470)
spaced(b2, "SEIRYU", 100, 0, 720, GOLD, ls=20, font=SERIF, cx0=470)
b2.save(f"{OUT}/seiryv_concept.png")

# ---- beat 3: wisdom 一陽来復 (4 chars) — over fr6 (wide samurai + sunrise dragon) ----
b3 = new()
scrim(b3, 1000, 1420, left=-140, right=880)
gold_kanji(b3, ["一", "陽", "来", "復"], 118, X, 1036, 128)
spaced(b3, "Ichiyo raifuku", 52, X + 4, 1226, GOLD, ls=3, font=SERIF)
ImageDraw.Draw(b3).text((X + 4, 1300), "One yang returns",
                        font=ImageFont.truetype(SERIF, 34), fill=SUB)
b3.save(f"{OUT}/seiryv_wisdom.png")

# ---- safe-zone check ----
ok = True
for name in ["seiryv_hook", "seiryv_concept", "seiryv_wisdom"]:
