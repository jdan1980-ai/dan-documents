#!/usr/bin/env python3
"""SUIRYU Short builder — 6 vertical shots, jitter-free Ken Burns (PIL float crop).

fr1 samurai + dragon head/neck rising, moon (hook) · fr2 mossy stone steps, crescent moon (climb)
fr3 dragon head close, teal spray (reveal) · fr4 milky way + moon, calm sky (concept 水龍 + SUIRYU)
fr5 golden waterfall closeup (breather) · fr6 samurai + full dragon, moon (wisdom / close)
"""
...
SHOTS = [
    ("suiryu-shorts-fr1.jpg", True),
    ("suiryu-shorts-fr2.jpg", False),
    ("suiryu-shorts-fr3.jpg", True),
    ("suiryu-shorts-fr4.jpg", False),
    ("suiryu-shorts-fr5.jpg", True),
    ("suiryu-shorts-fr6.jpg", False),
]
ZOOM = 0.10

# step = 5.7, total = 5.7*5 + 6.5 = 35.0s
OVERLAYS = [
    ("suiv_hook.png", 3.0, 8.4),        # shot 1
    ("suiv_concept.png", 18.0, 23.0),   # shot 4 (starts 17.1)
    ("suiv_wisdom.png", 29.6, 34.0),    # shot 6 (starts 28.5)
]
FADE = 0.9
END_FADE = 1.0
...
def load_cover(p):
    ...
    return im.resize((W * 2, H * 2), Image.LANCZOS)  # supersample so upscaled sources stay crisp
