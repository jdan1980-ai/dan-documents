#!/usr/bin/env python3
"""SUIRYU Short text overlays — transparent PNGs (1080x1920), cream + gold.
...
beat 1 hook (shot 1)    — "The carp that never stops becomes the dragon" (cream)
beat 2 concept (shot 4) — 水龍 + SUIRYU (gold) over the calm night sky
beat 3 wisdom (shot 6)  — 柔よく剛を制す / Ju yoku go o seisu / Softness overcomes hardness (gold)
"""
...
# ---- beat 1: hook (cream, SERIF) — over fr1 (samurai + dragon head, moon) ----
b1 = new()
scrim(b1, 968, 1360)
d = ImageDraw.Draw(b1)
f1 = ImageFont.truetype(SERIF, 74)
for i, line in enumerate(["The carp that", "never stops", "becomes the dragon"]):
    d.text((X, 1010 + i * 104), line, font=f1, fill=CREAM)
b1.save(f"{OUT}/suiv_hook.png")

# ---- beat 2: concept 水龍 + SUIRYU — over fr4 (calm night sky, milky way + moon) ----
b2 = new()
sc = Image.new("RGBA", (W, H), (0, 0, 0, 0))
ImageDraw.Draw(sc).rounded_rectangle((250, 300, 830, 900), radius=180, fill=(8, 7, 6, 150))
b2.alpha_composite(sc.filter(ImageFilter.GaussianBlur(110)))
gold_kanji(b2, ["水", "龍"], 220, 0, 380, 240, cx0=470)
spaced(b2, "SUIRYU", 96, 0, 720, GOLD, ls=18, font=SERIF, cx0=470)
b2.save(f"{OUT}/suiv_concept.png")

# ---- beat 3: wisdom 柔よく剛を制す (7 chars) — over fr6 (samurai + full dragon, moon) ----
b3 = new()
scrim(b3, 1000, 1420, left=-140, right=880)
gold_kanji(b3, ["柔", "よ", "く", "剛", "を", "制", "す"], 76, 0, 1036, 82, cx0=470)
spaced(b3, "Ju yoku go o seisu", 44, 0, 1210, GOLD, ls=2, font=SERIF, cx0=470)
w3 = ImageFont.truetype(SERIF, 32)
d3 = ImageDraw.Draw(b3)
txt = "Softness overcomes hardness"
tw = d3.textlength(txt, font=w3)
d3.text((470 - tw / 2, 1280), txt, font=w3, fill=SUB)
b3.save(f"{OUT}/suiv_wisdom.png")

# ---- safe-zone check ----
ok = True
for name in ["suiv_hook", "suiv_concept", "suiv_wisdom"]:
