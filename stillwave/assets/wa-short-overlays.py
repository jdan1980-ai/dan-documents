#!/usr/bin/env python3
"""WA Short text overlays — transparent PNGs (1080x1920), warm gold + cream.
...
beat 1 hook (shot 1)   — "When the water is still, the sky rests inside it" (cream)
beat 2 concept (shot 4)— 和 + WA (gold) over the clean misty sky
beat 3 wisdom (shot 6) — 和顔愛語 / Wagen aigo / A gentle face, kind words (gold)
"""
...
for i, line in enumerate(["When the water is", "still, the whole", "sky rests inside it"]):
    d.text((X, 1010 + i * 104), line, font=f1, fill=CREAM)
b1.save(f"{OUT}/wav_hook.png")

# ---- beat 2: concept 和 + WA — high over the clean misty sky ----
b2 = new()
...
gold_kanji(b2, ["和"], 300, 0, 330, 0, cx0=540)
spaced(b2, "WA", 128, 0, 720, GOLD, ls=26, font=SERIF, cx0=540)
b2.save(f"{OUT}/wav_concept.png")

# ---- beat 3: wisdom 和顔愛語 (4 chars) ----
b3 = new()
scrim(b3, 1030, 1420)
gold_kanji(b3, ["和", "顔", "愛", "語"], 116, X, 1066, 126)
spaced(b3, "Wagen aigo", 52, X + 4, 1230, GOLD, ls=3, font=SERIF)
ImageDraw.Draw(b3).text((X + 4, 1306), "A gentle face, kind words",
                        font=ImageFont.truetype(SERIF, 36), fill=SUB)
b3.save(f"{OUT}/wav_wisdom.png")

# ---- safe-zone check ----
ok = True
for name in ["wav_hook", "wav_concept", "wav_wisdom"]:
