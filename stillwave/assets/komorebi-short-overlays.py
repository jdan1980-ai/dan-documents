#!/usr/bin/env python3
"""KOMOREBI Short text overlays — transparent PNGs (1080x1920), warm gold + cream.
...
def scrim(img, top, bot, alpha=165):
    s = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    ImageDraw.Draw(s).rounded_rectangle((-140, top, 760, bot), radius=150, fill=(10, 14, 8, alpha))
...
for i, line in enumerate(["Sunlight", "through the leaves"]):
    d.text((X, 1040 + i * 98), line, font=f1, fill=CREAM)
b1.save(f"{OUT}/kov_hook.png")

# ---- beat 2: concept 木漏れ日 + KOMOREBI (shot 4 = canopy gap, lower third is dark) ----
b2 = new()
scrim(b2, 940, 1410)
gold_kanji(b2, ["木", "漏", "れ", "日"], 150, X, 970, 158)
spaced(b2, "KOMOREBI", 74, X + 4, 1210, GOLD, ls=8)
b2.save(f"{OUT}/kov_concept.png")

# ---- beat 3: wisdom 一期一会 ----
b3 = new()
scrim(b3, 960, 1400)
gold_kanji(b3, ["一", "期", "一", "会"], 118, X, 980, 128)
spaced(b3, "Ichigo ichie", 50, X + 4, 1146, GOLD, ls=3)
ImageDraw.Draw(b3).text((X + 4, 1228), "This moment, once, never again",
                        font=ImageFont.truetype(KANJI, 36), fill=SUB)
b3.save(f"{OUT}/kov_wisdom.png")
